...
def get_all_auctions_restricted():
    req = requests.get("https://localhost:5000/all?status=active", headers=generate_session_token_system())
...
def all_my_auction(user):
    ...
    req = requests.get("https://localhost:5000/get_auction?user_id=" + str(user_id),
                       headers=generate_session_token_system())
    return send_response(req.json(), req.status_code)

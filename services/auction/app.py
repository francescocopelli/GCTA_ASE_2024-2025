...
def get_all_auctions_restricted():
    req = requests.get("http://localhost:5000/all?status=active",  timeout=3, verify=False, headers=generate_session_token_system())
...
def all_my_auction(user):
    ...
    req = requests.get("http://localhost:5000/get_auction?user_id=" + str(user_id), timeout=3, verify=False, 
                       headers=generate_session_token_system())
    return send_response(req.json(), req.status_code)

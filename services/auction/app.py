...
def get_all_auctions_restricted():
    req = requests.get("https://localhost:5000/all?status=active",  timeout=10, headers=generate_session_token_system(),verify=False)
...
def is_gacha_unlocked(user_id, gacha_id):
    try:
        response = requests.get(f"{gacha_url}/is_gacha_unlocked/{user_id}/{gacha_id}", timeout=10, 
...
def update_gacha_status(user_id, gacha_id, status):
    try:
        response = requests.put(f"{gacha_url}/update_gacha_status", timeout=10, 
...
def update_gacha_owner(buyer_id, gacha_id, seller_id, status):
    try:
        response = requests.put(f"{gacha_url}/update_gacha_owner", timeout=10, 
...
def create_transaction(user_id, amount, transaction_type):
    try:
        response = requests.post(f"{transaction_url}/add_transaction", timeout=10, 
...
def update_user_balance(user_id, amount, type):
    try:
        response = requests.put(f"{user_url}/update_balance/PLAYER", headers=generate_session_token_system(), timeout=10, 
...
def get_user_balance(user_id):
    try:
        response = requests.get(f"{admin_url}/get_user_balance/{user_id}", timeout=10,  headers=generate_session_token_system(),verify=False)
        response.raise_for_status()
        logging.debug(f"Response from user service: {response.json()}")
        return send_response(response.json(), 200)
    except requests.exceptions.HTTPError as http_err:
...
def all_my_auction(user):
    ...
    req = requests.get("https://localhost:5000/get_auction?user_id=" + str(user_id), timeout=10, 
                       headers=generate_session_token_system(),verify=False)
    return send_response(req.json(), req.status_code)

...
def create_transaction(user_id, amount, transaction_type):
    response = requests.post(f"{transaction_url}/add_transaction", headers=generate_session_token_system(),verify=False, 
...
def my_gacha_list():
    user_id = str(
        jwt.decode(request.headers["Authorization"].split(" ")[1], app.config["SECRET_KEY"], algorithms=["HS256"])[
            "user_id"])
    response = requests.get(f"{gacha_url}/inventory/" + user_id, verify=False, headers=request.headers)
...
def gacha_info(user_id, gacha_id):
    response = requests.get(f"{gacha_url}/get/" + str(user_id) + "/" + str(gacha_id),verify=False, 
...
def update_user_balance(user_id, amount, type):
    response = requests.put(
        f"{dbm_url}/update_balance/PLAYER", headers=request.headers,verify=False, 
...
def get_user_balance(current_user):
    ...
    response = requests.get(f"{dbm_url}/balance/PLAYER", params={"user_id": user_id}, verify=False, headers=request.headers)
    return send_response(response.json(), response.status_code)
...
def get_user(user):
    ...
    response = requests.get(url, verify=False, headers=request.headers)
    return send_response(response.json(), response.status_code)
...
def update_balance(user_type):
    ...
    response = requests.put(url, verify=False, json=data, headers=request.headers)
...
def update(user):
    ...
    response = requests.put(url, verify=False, json=data, headers=generate_session_token_system())
    return send_response(response.json(), response.status_code)

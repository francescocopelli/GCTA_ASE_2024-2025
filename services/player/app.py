...
def my_gacha_list():
    user_id = str(
        jwt.decode(request.headers["Authorization"].split(" ")[1], app.config["SECRET_KEY"], algorithms=["HS256"])[
            "user_id"])
    response = requests.get(f"{gacha_url}/inventory/" + user_id,  timeout=3, headers=request.headers,verify=False)
...
def get_user_balance(current_user):
    ...
    response = requests.get(f"{dbm_url}/balance/PLAYER", params={"user_id": user_id},  timeout=3, headers=request.headers,verify=False)
    return send_response(response.json(), response.status_code)
...
def get_user(user):
    ...
    response = requests.get(url,  timeout=3, headers=request.headers,verify=False)
    return send_response(response.json(), response.status_code)
...
def get_user_by_id(user_id):
    ...
    response = requests.get(url,  timeout=3, headers=generate_session_token_system(),verify=False)
...
def update_balance(user_type):
    ...
    response = requests.put(url,  timeout=3, json=data, headers=request.headers,verify=False)

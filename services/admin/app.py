...
def update_myself(user):
    ...
    if jwt.decode(request.headers["Authorization"].split(" ")[1], app.config["SECRET_KEY"], algorithms=["HS256"])["user_type"] != "PLAYER":
        req = requests.get(f"{dbm_url}/get_user/{user_id}", headers=generate_session_token_system())
        user = req.json()
        if not user:
...
def update(user_id):
    ...
    user = requests.get(f"{dbm_url}/get_user/{user_type}/{user_id}", headers=generate_session_token_system()).json()
    if not user:
        return send_response({"error": "User not found"}, 404)

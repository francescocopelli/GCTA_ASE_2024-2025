...
def update_myself(user):
    ...
    if jwt.decode(request.headers["Authorization"].split(" ")[1], app.config["SECRET_KEY"], algorithms=["HS256"])["user_type"] != "PLAYER":
        req = requests.get(f"{dbm_url}/get_user/{user_id}", verify=False, headers=generate_session_token_system())
        user = req.json()
        if not user:
...
def update(user_id):
    ...
    user = requests.get(f"{dbm_url}/get_user/{user_type}/{user_id}",verify=False,  headers=generate_session_token_system()).json()
    if not user:
        return send_response({"error": "User not found"}, 404)
...
def get_user_balance_admin(user_id):
    response = requests.get(f"{dbm_url}/balance/PLAYER", verify=False,  params={"user_id": user_id}, headers=request.headers)
    return send_response(response.json(), response.status_code)

...
def update_myself(user):
    ...
    if jwt.decode(request.headers["Authorization"].split(" ")[1], app.config["SECRET_KEY"], algorithms=["HS256"])["user_type"] != "PLAYER":
        req = requests.get(f"{dbm_url}/get_user/{user_id}",  timeout=3, headers=generate_session_token_system(),verify=False)
        user = req.json()
        if not user:
...
def update(user_id):
    ...
    user = requests.get(f"{dbm_url}/get_user/{user_type}/{user_id}", timeout=3,  headers=generate_session_token_system(),verify=False).json()
    if not user:
        return send_response({"error": "User not found"}, 404)
...
def get_user_balance_admin(user_id):
    response = requests.get(f"{dbm_url}/balance/PLAYER",  timeout=3,  params={"user_id": user_id}, headers=request.headers,verify=False)
    return send_response(response.json(), response.status_code)

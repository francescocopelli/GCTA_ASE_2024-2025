...
def _f(require_return, f, *args, **kwargs):
    ...
    try:
        ...
        rst = requests.get(f"{dbm_url}/get_user/{data['user_type']}/{data['user_id']}",verify=False, timeout=3, 
...
def admin_required(f):
    def admin_f(require_return, f, *args, **kwargs):
        ...
        try:
            ...
            rst = requests.get(f"{dbm_url}/get_user/ADMIN/{user_id}",verify=False, timeout=3, 
                               headers=generate_session_token_system())
